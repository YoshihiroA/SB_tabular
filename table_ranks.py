...
6. Computes TOTAL RANK SUM per model (equally weighted metrics)
7. Creates FINAL TIER LIST with average model rank across datasets
8. Handles ties: models with equal rank sum get same tier position
...
# ============================================================================
# CREATE FINAL TIER LIST
# ============================================================================

def create_final_tier_list(all_results: Dict) -> pd.DataFrame:
    """
    Create final tier list with average model rank across datasets.
    Models with equal rank sum get same tier position (dense ranking).
    
    Returns:
        DataFrame with tier list
    """
    print(f"\n\n{'='*80}")
    print("FINAL TIER LIST - MODEL RANKINGS ACROSS ALL DATASETS")
    print('='*80)
    
    # Collect all rank sums per model
    model_rank_sums = {model: [] for model in MODELS}
    
    for dataset_name in sorted(all_results.keys()):
        rank_table = all_results[dataset_name]["rank_table"]
        rank_sums = rank_table.sum(axis=1)
        for model in MODELS:
            if model in rank_sums.index:
                val = rank_sums[model]
                if not np.isnan(val):
                    model_rank_sums[model].append(val)
    
    # Calculate average rank sum per model
    avg_rank_sums = {}
    for model, sums in model_rank_sums.items():
        if sums:
            avg_rank_sums[model] = np.mean(sums)
        else:
            avg_rank_sums[model] = np.nan
    
    # Sort by average rank sum (ascending)
    sorted_models = sorted(
        [(m, s) for m, s in avg_rank_sums.items() if not np.isnan(s)],
        key=lambda x: x[1]
    )
    ...
    for idx, (model, avg_sum) in enumerate(sorted_models):
        # If rank sum changed, increment tier (dense ranking)
        if prev_sum is not None and avg_sum != prev_sum:
            current_tier = idx + 1
        
        num_datasets = len(model_rank_sums[model])
        tier_data.append({
            "Tier": current_tier,
            "Model": model,
            "Avg Rank Sum": f"{avg_sum:.2f}",
    ...
    # Legend
    print(f"\n{'─'*80}")
    print("Tier Assignment (Dense Ranking with Ties):")
    print("  - Models with equal Avg Rank Sum get same tier")
    print("  - Next tier number follows sequentially (no gaps in rank positions)")
    print("  - Lower Avg Rank Sum = Better performance")
    print(f"{'─'*80}\n")
...
def main():
    ...
    print("\n" + "="*80)
    print("RANKING TABLES WITH WILCOXON TEST (PER DATASET) + FINAL TIER LIST")
    print("="*80)
    ...
    # Create final tier list
    tier_df = create_final_tier_list(all_results)
    
    # Save tier list
    tier_df.to_csv("ranking_tables/final_tier_list.csv", index=False)
    print(f"✓ Tier list saved: ranking_tables/final_tier_list.csv")
    
    print(f"\n{'='*80}")
    print(f"✓ All {len(DATASETS)} ranking table PNGs created in 'ranking_tables/' directory")
    print(f"✓ Final tier list with dense ranking created")
    ...
    print(f"✓ Tier positions: Equal rank sums get same tier, next model gets next tier number")
    print(f"{'='*80}\n")
