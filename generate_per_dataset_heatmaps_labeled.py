...
- Rows: Models (5 total: DSBM, ASBM, XGBoost-DSBM, DSBM-VAE, TabSyn) - WITH LABELS
- Columns: Metrics (5 total: R², RMSE, Corr. Dist, KL Div, Wasserstein) - WITH LABELS
...
def plot_dataset_heatmap(dataset: str, df: pd.DataFrame, 
    ...
    Features:
    - Model names as row labels (Y-axis)
    - Metric names as column labels (X-axis)
    - Outliers marked with gray and asterisk
    - Color scale based on non-outlier values
    """
    ...
    # Set tick labels with actual names
    model_names = list(MODEL_LABELS.values())
    metric_names = [METRICS_TO_PLOT[m]["label"] for m in METRICS_TO_PLOT.keys()]
