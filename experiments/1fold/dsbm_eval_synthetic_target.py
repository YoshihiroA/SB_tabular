...
"""
DSBM Complete Evaluation Pipeline (FULL DATA WITH SYNTHETIC TARGET)

Workflow:
1. Load dataset (features + target as last column)
2. Split: (X_train, y_train) | (X_test, y_test)
3. Merge: X_train_full = [X_train | y_train] for DSBM input
4. Train DSBM on FULL data (features + target together)
...
Output:
- dsbm_evaluation_results.json (9 metrics)
- dsbm_evaluation_plots/ (9 pairs + 3x3 grid)
"""
...
# ============================================================================
# 1. LOAD AND PARSE DSBM HYPERPARAMETERS
# ============================================================================

def load_dsbm_params(params_file: str = "optuna_results_dsbm/SUMMARY.json") -> Dict:
    """Load best hyperparameters from DSBM params JSON."""
    params_path = Path(params_file)
    if not params_path.exists():
        raise FileNotFoundError(f"DSBM params file not found: {params_file}")

    with open(params_path, 'r') as f:
        params = json.load(f)

    print(f"✓ Loaded DSBM params: {params_file}")
...
# ============================================================================
# 3. CREATE AND TRAIN DSBM BRIDGE (ON FULL DATA)
# ============================================================================

def create_and_train_dsbm_full(
    ...
    """Create DSBMTabularBridge and train on FULL data (features + target)."""
    m_train, n_cols = X_train_full.shape
    m_test = X_test_full.shape[0]

    if verbose:
        print(f"✓ Creating DSBM for {dataset_name} (FULL data: features + target)")
    ...
    if verbose:
        print(f"✓ Training DSBM (timesteps={best_params['num_timesteps']}, "
              f"lr={best_params['lr']:.2e}, sigma={best_params['sigma']:.4f})...")
...
def compute_catboost_utility_metrics(
    ...
    X_synth_full = bridge.generate(n_samples=len(X_train_full))
...
def evaluate_all_dsbm_models(
    params_file: str = "optuna_results_dsbm/SUMMARY.json",
    pkl_file: str = "datasets_numeric_merged.pkl",
    output_json: str = "dsbm_evaluation_results.json",
    output_plots_dir: str = "dsbm_evaluation_plots",
    ...
    print("\n" + "="*80)
    print("DSBM EVALUATION (WITH SYNTHETIC TARGET)")
    print("="*80 + "\n")

    # Load DSBM params
    params_dict = load_dsbm_params(params_file)
    ...
    for i, dataset_name in enumerate(dataset_names, 1):
        ...
        try:
            if dataset_name not in params_dict:
                print(f"⚠️ No DSBM params for {dataset_name}\n")
                continue

            params_result = params_dict[dataset_name]

            if 'error' in params_result:
                print(f"⚠️ Params failed: {params_result['error']}\n")
                continue
            ...
            bridge = create_and_train_dsbm_full(
...
def create_metric_plots(results: Dict[str, Dict], output_dir: str):
    ...
    for metric_real, metric_synth in metrics_pairs:
        ...
        plt.savefig(output_path / f"dsbm_comparison_{plot_name}.png", dpi=300)
        plt.close()
    ...
    plt.suptitle('DSBM Evaluation: Real vs Synthetic Models (3x3 Grid)',
                 fontsize=16, fontweight='bold', y=0.995)
    plt.tight_layout()
    plt.savefig(output_path / "dsbm_comparison_grid_3x3.png", dpi=300)
    plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--params-file", default="results/optuna_results/optuna_results_dsbm/SUMMARY.json")
    ...
    evaluate_all_dsbm_models(
        params_file=args.params_file,
        pkl_file=args.pkl_file,
        dataset_names=args.datasets,
        output_plots_dir="dsbm_evaluation_plots" if not args.no_plots else None
    )
