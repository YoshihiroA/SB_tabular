...
"""
optuna_tune_all_datasets_dsbm.py

Hyperparameter tuning for DSBM on ALL datasets from merged pickle file.

Saves best params for EACH dataset to separate JSON files in optuna_results_dsbm/ folder.

ARCHITECTURE: Preserves logic from ASBM version, adapted for DSBM hyperparameters.

DSBM Parameters:
  - num_timesteps: [100, 200] (DSBM typically needs more timesteps than ASBM)
  - lr: Single learning rate (not separated lr_g/lr_d like ASBM)
  - sigma: Scale/diffusion parameter [0.01, 2.0]
  - layers: Single layer architecture [256-512 x 3 layers]

Usage:

python optuna_tune_dsbm_all_datasets.py # Tune all, 20 trials each

python optuna_tune_dsbm_all_datasets.py --n-trials 50 # More trials

python optuna_tune_dsbm_all_datasets.py --pkl-file custom.pkl

python optuna_tune_dsbm_all_datasets.py --datasets wine california_housing
...
try:
    ...
    print("❌ Error: dsbm_tabular_bridge.py not found")
...
def validate_dimensions(X_train: np.ndarray, X_test: np.ndarray, dataset_name: str, batch_size: int) -> None:
    """Validate training data dimensions before DSBM initialization."""
...
def objective(
    ...
    """
    Optuna objective function for DSBM tuning.

    Suggests hyperparameters and trains DSBM, returns metric.
    """

    # ========================================================================
    # DSBM-SPECIFIC HYPERPARAMETERS
    # ========================================================================

    # Number of timesteps (DSBM typically needs more than ASBM)
    num_timesteps = trial.suggest_int('num_timesteps', 100, 200, step=25)

    # Single learning rate (not separated like ASBM)
    lr = trial.suggest_float('lr', 1e-6, 1e-3, log=True)

    # Diffusion/scale parameter
    sigma = trial.suggest_float('sigma', 0.01, 2.0, log=True)

    # Network layers (single architecture for DSBM, not separate G/D like ASBM)
    layers = [
        trial.suggest_int('layer_0', 128, 512, step=128),
        trial.suggest_int('layer_1', 128, 512, step=128),
        trial.suggest_int('layer_2', 128, 512, step=128),
    ]

    if verbose and trial.number % 5 == 0:
        print(f"\n Trial {trial.number}:")
        print(f" num_timesteps={num_timesteps}, lr={lr:.2e}, sigma={sigma:.4f}")
    ...
    try:
        ...
        # ====================================================================
        # DSBM INITIALIZATION (Different from ASBM)
        # ====================================================================
        ...
        # ====================================================================
        # DSBM TRAINING (Different from ASBM)
        # ====================================================================
        ...
        synth_w = bridge.generate(n_samples=m_train)
...
def save_dataset_results(
    ...
    # Extract layers from individual layer_* entries
    layers = [
        int(best_params["layer_0"]),
        int(best_params["layer_1"]),
        int(best_params["layer_2"]),
    ]
...
def tune_all_datasets(
    ...
    results_dir: str = "optuna_results_dsbm",
    verbose: bool = False,
    ...
    print("\n" + "="*80)
    print("OPTUNA HYPERPARAMETER TUNING FOR DSBM")
    print("="*80)
    ...
    for i, dataset_name in enumerate(dataset_names, 1):
        ...
        try:
            ...
            study = optuna.create_study(
                direction='minimize',
                sampler=sampler,
                study_name=f"dsbm_{dataset_name}"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Tune DSBM on all datasets, save individual results")

    parser.add_argument(
        "--pkl-file",
        type=str,
        default="datasets/datasets_numeric.pkl",
...
""")
    ...
    print("\nUsage in code:")
    print(f"""
from optuna_tune_dsbm_all_datasets import load_dataset_params
import json
